...
namespace ManyWho.Service.Docordo.Utils
{
    public class CanvasUtils
    {
        ...
        private static String[] GetParts(IAuthenticatedWho authenticatedWho, String input)
        {
            ...
            if (input == null || 
                input.IndexOf(".") <= 0)
            {
                throw ErrorUtils.GetWebException(HttpStatusCode.BadRequest, "Input [" + input + "] doesn't look like a signed request");
            }
        ...
        private static void Verify(IAuthenticatedWho authenticatedWho, String secret, String encodedEnvelope, String encodedSig)
        {
            ...
            {
                throw ErrorUtils.GetWebException(HttpStatusCode.BadRequest, "Encoded envelope is null or blank.");
            }

            if (encodedSig == null ||
                encodedSig.Trim().Length == 0)
            {
                throw ErrorUtils.GetWebException(HttpStatusCode.BadRequest, "Encoded signature is null or blank.");
            }

            if (secret == null || 
                secret.Trim().Length == 0)
            {
                throw ErrorUtils.GetWebException(HttpStatusCode.BadRequest, "Secret is null, did you set your environment variable CANVAS_CONSUMER_SECRET?");
            }
